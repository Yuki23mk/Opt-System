/**
 * ファイルパス: OptiOil-API/pages/api/admin/orders/[id]/reject-cancel.ts
 * 管理者用 - キャンセル拒否API
 */
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!reason || !reason.trim()) {
      return res.status(400).json({ error: '拒否理由が必要です' });
    }
    ...
    console.log(`✅ 管理者：キャンセル拒否 ${orderId} 理由: ${reason}`);
    return res.status(200).json({ 
      message: 'キャンセル申請を拒否しました',
      order: updatedOrder
    });

  } catch (error) {
    console.error('❌ 管理者キャンセル拒否API エラー:', error);
